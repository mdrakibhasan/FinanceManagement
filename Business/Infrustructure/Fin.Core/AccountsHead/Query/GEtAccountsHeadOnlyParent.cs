...
{
    public class GEtAccountsHeadOnlyParent : IRequest<IEnumerable<VmAccountsHead>>;
   
    public class GetAccountsHeadAllHandler : IRequestHandler<GEtAccountsHeadOnlyParent, IEnumerable<VmAccountsHead>>
    {
        ...
        [UsedImplicitly]
        public async Task<IEnumerable<VmAccountsHead>> Handle(GEtAccountsHeadOnlyParent request, CancellationToken cancellationToken)
        {

            var data = await _sateRepository.GetAccountsTypeOnlyRoot();
